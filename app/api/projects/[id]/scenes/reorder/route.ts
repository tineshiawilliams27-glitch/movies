...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const projectScenes = await db.select({ id: scenes.id }).from(scenes).where(and(eq(scenes.projectId, id), eq(scenes.userId, session.user.id)))
  if (projectScenes.length !== parsed.data.sceneIds.length || projectScenes.some(({ id: sceneId }) => !parsed.data.sceneIds.includes(sceneId))) return NextResponse.json({ error: 'Scene order must include every project scene exactly once.' }, { status: 400 })
