  ...
  if (parsed.data.kind === 'pipeline') {
    ...
    const { jobs } = await db.transaction(async (tx) => {
      ...
      for (const shot of output.shots) {
        ...
        const videoPayload = { userId: session.user.id, projectId: id, shotNumber: shot.shotNumber, prompt: continuityPrompt, durationSeconds: shot.durationSeconds, shotType: shot.shotType, cameraMovement: shot.cameraMovement, lighting: shot.lighting, mood: shot.mood, aspectRatio: '16:9', quality: '768p', styleBible, characterReferences: referencedCharacters, sceneContext, previousShotContext, dependsOnJobIds: dependencyIds }
