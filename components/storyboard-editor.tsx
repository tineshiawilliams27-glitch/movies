...
export function StoryboardEditor({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    ...
    async function loadScenes() {
      ...
      setLoadError('')
      try {
        const response = await fetch(`/api/projects/${projectId}/scenes`, { cache: 'no-store' })
        const data = await response.json()
        if (!cancelled && response.ok) setScenes(data.scenes ?? [])
        if (!cancelled && !response.ok) setLoadError(data.error || 'Unable to load storyboard')
      } catch { if (!cancelled) setLoadError('Unable to load storyboard. Check your connection.') }
