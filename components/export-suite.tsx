...
export function ExportSuite({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    ...
    const loadLatestJob = async () => {
      try {
        ...
        if (!response.ok) {
          if (active) setPollingError('Live render updates are unavailable. Refresh to check the latest status.')
          return
        }
  ...
  async function downloadManifest() {
    ...
    try {
      ...
      if (!response.ok) throw new Error('The export manifest could not be generated. Some project data may be incomplete.')
      ...
      setError(manifestError instanceof Error ? manifestError.message : 'The export manifest failed. Please retry.')
  ...
  async function startExport() {
    ...
    try {
      ...
      const response = await fetch(`/api/projects/${projectId}/jobs`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'VIDEO_EXPORT', idempotencyKey: `export:${projectId}:${settings.format}:${settings.resolution}:${settings.frameRate}:${settings.aspectRatio}`, payload: settings }), signal: controller.signal })
      window.clearTimeout(timeout)
      if (!response.ok) {
        const data = await response.json().catch(() => null) as { error?: string } | null
        setStatus('error')
        setError(data?.error || 'The export could not be queued. Check that media and audio are available, then try again.')
        return
      }
      const data = await response.json()
      if (!data.job?.id) {
