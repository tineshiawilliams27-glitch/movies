...
export function ExportSuite({ projectId }: { projectId: string }) {
  ...
  async function startExport() {
    ...
    setError('')
    try {
      const response = await fetch(`/api/projects/${projectId}/jobs`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'VIDEO_EXPORT', idempotencyKey: `export:${projectId}:${settings.format}:${settings.resolution}:${settings.frameRate}:${settings.aspectRatio}`, payload: settings }) })
      if (!response.ok) {
