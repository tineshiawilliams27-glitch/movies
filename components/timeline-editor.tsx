...
export function TimelineEditor({ projectId }: { projectId: string }) {
    ...
    async function loadTimeline() {
      ...
      try {
        ...
        if (filmResponse.ok) {
          ...
          setLoadError(data?.error || 'Unable to load timeline data')
        }
        if (scenesResponse.ok) {
          ...
          setLoadError(data?.error || 'Unable to load scenes')
        }
      } catch {
        if (!cancelled) setLoadError('Unable to load timeline data. Check your connection.')
      } finally {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => setCursor((value) => value >= totalSeconds ? 0 : value + 0.25), 250)
