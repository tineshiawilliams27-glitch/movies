...
export function ProjectWorkspace({ projectId }: { projectId: string }) {
  ...
  async function generatePipeline() {
    ...
    setMessage(response.ok ? 'Pipeline generated' : 'Pipeline failed')
  ...
  return <main className="min-h-screen bg-background text-foreground">
    ...
    <div className="mx-auto max-w-[1500px] px-5 py-6 md:px-8"><WorkspaceNavigation projectId={projectId} /><div className="mb-6 mt-6 flex items-center justify-between"><div><p className="text-xs font-medium uppercase tracking-[0.18em] text-accent">Scene builder</p><h2 className="mt-2 font-serif text-3xl">Shape the next beat.</h2></div><div className="flex items-center gap-2"><button onClick={generatePipeline} className="inline-flex items-center gap-2 rounded-lg bg-primary px-3 py-2 text-sm font-medium text-primary-foreground"><Sparkles size={15} />Generate full pipeline</button><button onClick={addScene} className="inline-flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm hover:bg-accent"><Plus size={15} />New scene</button></div></div>
