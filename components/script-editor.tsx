...
export function ScriptEditor({ projectId }: { projectId: string }) {
  ...
  async function save() {
    ...
    setSaving(true)
    setSaved(false)
    const response = await fetch(`/api/projects/${projectId}`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify(project) })
    setSaving(false)
