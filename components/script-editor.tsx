...
type Project = { title?: string | null; concept?: string | null; format?: string | null; metadata?: Record<string, unknown> | null }
...
export function ScriptEditor({ projectId }: { projectId: string }) {
  ...
  async function save() {
    ...
    setSaveError('')
    try {
      const response = await fetch(`/api/projects/${projectId}`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ title: (project.title ?? '').trim(), concept: project.concept ?? '', format: project.format ?? 'Story', metadata: project.metadata && typeof project.metadata === 'object' && !Array.isArray(project.metadata) ? project.metadata : {} }) })
