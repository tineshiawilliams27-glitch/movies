      ...
      <div className="flex min-h-screen">
        <aside className={`${mobileOpen ? 'translate-x-0' : '-translate-x-full'} fixed inset-y-0 left-0 z-40 flex w-72 flex-col border-r border-border bg-sidebar px-4 py-5 transition-transform lg:static lg:translate-x-0`}>
          ...
          <div className="mt-8 rounded-xl border border-border bg-card/70 p-2">
            <button className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left hover:bg-accent" aria-label="Open project switcher">
          ...
          <div className="mt-auto flex flex-col gap-1">
            ...
            <button className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm text-muted-foreground hover:bg-accent hover:text-foreground"><CircleHelp size={17} />Help center</button>
          </div>
        </aside>
        ...
        <section className="min-w-0 flex-1">
          ...
          <div className="mx-auto max-w-[1500px] px-5 py-8 md:px-8 lg:px-10">
            <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><div className="mb-3 flex items-center gap-2 text-xs font-medium text-accent"><span className="size-1.5 rounded-full bg-accent" />WORKSPACE OVERVIEW</div><h1 className="font-serif text-4xl tracking-tight md:text-5xl">Make something <em className="text-primary not-italic">worth watching.</em></h1><p className="mt-3 max-w-xl text-sm leading-6 text-muted-foreground">A quiet place to turn loose ideas into full-length stories, scenes, and finished films.</p></div><button onClick={() => setShowCreate(true)} className="inline-flex items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-medium text-primary-foreground shadow-lg shadow-primary/10 hover:opacity-90"><Plus size={17} />Create video</button></div>

            <div className="mt-10 grid gap-4 md:grid-cols-3"><div className="rounded-xl border border-border bg-card p-5"><div className="flex items-center justify-between"><span className="text-sm text-muted-foreground">Active productions</span><Activity size={16} className="text-accent" /></div><div className="mt-4 flex items-end gap-2"><span className="font-serif text-3xl">{projects.length}</span><span className="pb-1 text-xs text-primary">+2 this month</span></div></div><div className="rounded-xl border border-border bg-card p-5"><div className="flex items-center justify-between"><span className="text-sm text-muted-foreground">Scenes in progress</span><Layers3 size={16} className="text-accent" /></div><div className="mt-4 flex items-end gap-2"><span className="font-serif text-3xl">73</span><span className="pb-1 text-xs text-muted-foreground">across all projects</span></div></div><div className="rounded-xl border border-border bg-card p-5"><div className="flex items-center justify-between"><span className="text-sm text-muted-foreground">Render queue</span><Clock3 size={16} className="text-accent" /></div><div className="mt-4 flex items-end gap-2"><span className="font-serif text-3xl">01</span><span className="pb-1 text-xs text-accent">processing now</span></div></div></div>

            <div className="mt-12 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-serif text-2xl">Recent projects</h2><p className="mt-1 text-sm text-muted-foreground">Pick up where you left off.</p></div><div className="flex items-center gap-2"><label className="flex items-center gap-2 rounded-lg border border-border bg-card px-3 py-2 text-sm text-muted-foreground"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search projects" className="w-32 bg-transparent outline-none placeholder:text-muted-foreground/60 sm:w-44" /></label><button className="rounded-lg border border-border bg-card p-2 text-muted-foreground hover:bg-accent" aria-label="Sort projects"><MoreHorizontal size={18} /></button></div></div>

            <div className="mt-5 grid gap-5 xl:grid-cols-3">{filteredProjects.map((project) => <article key={project.id} className="group overflow-hidden rounded-xl border border-border bg-card transition hover:border-primary/30 hover:shadow-xl hover:shadow-primary/5"><div className="relative aspect-[16/10] overflow-hidden"><img src={project.image} alt="" className="size-full object-cover transition duration-500 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-foreground/75 via-transparent to-transparent" /><div className="absolute bottom-3 left-3"><StatusBadge status={project.status} /></div><Link href={`/projects/${project.id}`} className="absolute right-3 top-3 flex size-9 items-center justify-center rounded-full bg-background/80 text-foreground opacity-0 backdrop-blur transition group-hover:opacity-100" aria-label={`Open ${project.title}`}><ArrowUpRight size={16} /></Link></div><div className="p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="font-medium tracking-tight">{project.title}</h3><p className="mt-1 text-xs text-muted-foreground">{project.type}</p></div><button className="rounded-md p-1 text-muted-foreground hover:bg-accent" aria-label={`More actions for ${project.title}`}><MoreHorizontal size={16} /></button></div><div className="mt-5 flex items-center justify-between border-t border-border pt-3 text-xs text-muted-foreground"><span>{project.duration} · {project.scenes} scenes</span><span>{project.updated}</span></div><div className="mt-4 flex items-center gap-2"><Link href={`/projects/${project.id}`} className="flex-1 rounded-lg bg-primary px-3 py-2 text-center text-xs font-medium text-primary-foreground hover:opacity-90">Continue editing</Link><button onClick={() => duplicateProject(project)} className="rounded-lg border border-border p-2 text-muted-foreground hover:bg-accent" aria-label={`Duplicate ${project.title}`}><Copy size={15} /></button><button onClick={() => deleteProject(project.id)} className="rounded-lg border border-border p-2 text-muted-foreground hover:bg-destructive/10 hover:text-destructive" aria-label={`Delete ${project.title}`}><Trash2 size={15} /></button></div></div></article>)}</div>
