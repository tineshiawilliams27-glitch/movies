...
The worker authenticates internal API requests with `Authorization: Bearer <WORKER_API_SECRET>`. Provider adapters use their named credentials only; legacy aliases are not supported.
...
- `GET /ready` requires `Authorization: Bearer <WORKER_API_SECRET>` and is suitable for an authenticated readiness check.
